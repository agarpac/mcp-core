...
describe('AdvancedInstaller', () => {
    it('renders installer form and POSTs to /api/install on click', async () => {
        ...
        await waitFor(() => expect(global.fetch).toHaveBeenCalledWith('/api/install', expect.objectContaining({ method: 'POST' })));
    });
});
//# sourceMappingURL=AdvancedInstaller.test.js.map