...
import { getToken, resetTokenCache, fetchSystem, fetchServers, fetchClients, install, uninstall, toggleClient, } from './client';
...
describe('api/client - endpoint wrappers', () => {
    ...
    it('fetchSystem sends Authorization: Bearer <token>', async () => {
        global.fetch.mockResolvedValueOnce(mockOk({ os: 'darwin' }));
        await fetchSystem();
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/system'), expect.objectContaining({
            headers: expect.objectContaining({ Authorization: `Bearer ${TOKEN}` }),
        }));
    });
    it('fetchServers returns parsed JSON', async () => {
        global.fetch.mockResolvedValueOnce(mockOk({ foo: { command: 'npx' } }));
        const res = await fetchServers();
        expect(res).toEqual({ foo: { command: 'npx' } });
    });
    it('fetchClients returns parsed JSON', async () => {
        global.fetch.mockResolvedValueOnce(mockOk([{ name: 'cursor', status: 'Installed', configPath: '/p', enabled: true }]));
        const res = await fetchClients();
        expect(res[0].name).toBe('cursor');
    });
    it('install POSTs the body as JSON with auth header', async () => {
        global.fetch.mockResolvedValueOnce(mockOk({ success: true }));
        await install({ source: 'foo', name: 'bar' });
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/install'), expect.objectContaining({
            method: 'POST',
    ...
    it('uninstall POSTs the name', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/uninstall'), expect.objectContaining({
            method: 'POST',
    ...
    it('toggleClient POSTs the triple', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/toggle-client'), expect.objectContaining({
            method: 'POST',
            body: JSON.stringify({ serverName: 's', clientName: 'cursor', enable: true }),
        }));
    });
    it('throws with message from server on !ok responses', async () => {
        global.fetch.mockResolvedValueOnce(mockErr(401, { error: 'Unauthorized' }));
